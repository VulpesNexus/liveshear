...
2. Download *LiveShear-0.1.1.zip* from the [latest release](../../releases/latest) and put *LiveShear.aip* in `%LOCALAPPDATA%\Adobe Illustrator Plug-ins\30`, creating that folder if it is not there. The *30* is Illustrator 2026's version number, and next year's Illustrator gets its own folder beside it.
...
- **Axis** is the direction the shear runs along, chosen the way Illustrator's own *Shear* dialog chooses it. *Horizontal* is the familiar italic slant and *Vertical* runs it up the page; both lock the **Axis Angle** slider at 0° or 90°. *Angle* unlocks the slider for any direction in between, and keeps what you typed if you look at *Horizontal* or *Vertical* and come back. The arrow keys move between the three. An axis of φ and one of φ + 180° describe the same shear.
- **Rotated artwork** keeps its slant only if the axis turns with it. The effect is applied after the object's own rotation, so the shear stays lined up with the page. After rotating by some angle, select *Angle*, set **Axis Angle** to that angle, and leave **Shear Angle** as it was. For a quarter turn either way, that is *Vertical*.
...
- A new Shear opens with the angles and axis you last committed, like Illustrator's own dialogs. Editing a Shear from the *Appearance* panel opens with that effect's own values instead. The angles are kept until Illustrator quits, and the *Preview* box stays the way you left it, across restarts too. *Reset* goes back to no shear along *Horizontal*.
