...
## Pattern fills are untested, because Illustrator would not draw one to test

Gradients are verified in pixels: the artwork is rendered by the effect and by *Object > Transform > Shear*, exported at the same size, and compared pixel by pixel, with no difference at all. The same test on a pattern fill proves nothing, because a pattern swatch built through Illustrator's scripting interface does not render — not sheared, not unsheared, not at all. The object is there and reports a `PatternColor` fill; the page comes out blank. Three ways of building the tile were tried, including the grouped bounding-box form Adobe's own documentation describes.

So there is no fixture to compare, and the probe says UNTESTED rather than comparing two empty pages and calling it agreement. **Affected:** anyone shearing pattern-filled artwork. **Source art is safe** regardless; this is about whether the pattern inside the shape leans with it. **Workaround:** none needed if it works — the effect passes Illustrator the flags that ask for fill and stroke patterns to be transformed, and its bounds match the native command on the same artwork. **Planned:** a pattern-filled document saved by hand and checked into the fixtures would settle it.
