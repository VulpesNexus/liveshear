...
- **Illustrator itself crashes under sustained scripted document churn.** The plugin is not necessary for it, and having it loaded does not detectably change how often it happens — three arms of six trials each, no difference between any pair. An experiment that size excludes a large influence, not a small one.
...
**The recorded commit is the published one.** The build record names `404e16a86740249f607cc7dc2cbcdddb3b772a1f`, *Release 0.1.3*, built on a clean tree. This repository's history was rewritten on 2026-09-15 to remove absolute paths that earlier evidence had recorded, which is why the 0.1.1 record named a hash that no longer exists; this commit came after that rewrite, so the hash in the record is the hash on GitHub.
...
The arithmetic underneath is checked separately and without a host: **3,459 checks**, covering the matrix at every angle and axis, the determinant, the reference point as a fixed point, the exact extent of cubic Béziers against a hundred thousand samples each, the angle sanitizing, the dialog layout at five display scales, and which axis button an angle selects and which values the dialog opens with. [evidence/mathtest.txt](evidence/mathtest.txt).
...
**The axis is chosen the way Illustrator's own *Shear* dialog chooses it**, new in 0.1.2: *Horizontal*, *Vertical*, and *Angle* buttons above the *Axis Angle* row. Like Illustrator's, the buttons are names for two angles rather than new state, so a document saved by 0.1.2 holds the same keys 0.1.1 wrote. The probe checks that *Vertical* and *Horizontal* set the axis and shear the artwork that way with the angle row locked, that *Angle* gives back the angle it held after a look at *Horizontal*, that Cancel leaves the stored axis alone whatever buttons were pressed, that the arrow keys move between the buttons and wrap, that the arrow keys on *Preview*, *Reset*, *Cancel*, and *OK* do not move the axis, and that *Reset* goes back to *Horizontal* with no shear. Which button opens selected is derived from the stored axis, and only the exact angles name one: an axis of −90° shears like 90° but opens under *Angle*, so pressing OK does not rewrite it.

Why the buttons matter was measured in Illustrator during development rather than argued, and that measurement is not part of the suite. The effect runs after the object's own rotation, and Illustrator sends a live effect no notice of a rotation — the message that reaches it on a transform carries a scale factor and nothing else — so the shear stays lined up with the page. Setting the axis to the rotation angle keeps the slant: at 90°, −90°, and 37°, the result matched the original sheared and then rotated to within 0.000001 pt, while an axis left at 0° missed by 7 to 18 pt. What the suite does keep is the premise: the release matrix's `rotatedRect` case, a rectangle rotated 37° and then sheared along an axis of 0°, matches Illustrator's own command, which shears along the page.

**A new Shear opens with the values last committed**, as Illustrator's own dialogs do, and Illustrator does not provide that: the menu probe measures it handing a new instance the default parameters straight after a commit at 20°. So the plugin remembers the angles and the axis itself, until Illustrator quits, and the *Preview* box goes into Illustrator's preferences and survives a restart. That is the same split Illustrator's preferences file keeps for its own transform dialogs, a *Preview* flag each and no values. The menu probe checks that a new Shear opens on the last commit and that Cancel is not remembered; the dialog probe checks that an effect edited from the *Appearance* panel opens on its own values rather than the remembered ones, and that *Preview* opens the way it was left. Because the dialog now changes a person's session, both probes save and restore what it remembers, and the suite ticks *Preview* for its run and puts the person's own setting back afterward. **32 checks in the dialog probe and 11 in the menu probe, none failed.** [evidence/dialog.txt](evidence/dialog.txt), [evidence/menu.txt](evidence/menu.txt).
...
**The About window had the same defect and kept it for one release longer.** The fix above reached the Shear dialog and not the About box, because the two windows each carried their own copy of the placement arithmetic — so the About box went on centering on its owner with nothing to keep it on the desktop, which matters more there than here: its prose does not scroll, so a clipped one can only be dragged, not read. Both windows now place themselves from one function, in *plugin/Source/DialogPlacement.h*, and there is no second copy left to forget. The same file is in *Subgroup*, which shows the same window.

That window's placement is now measured too, and it never had been — [evidence/about-placement.tsv](evidence/about-placement.tsv), **15 checks, none failed**. It needs no Illustrator: where the box opens depends only on the host window's rectangle and the monitor that rectangle falls on, so *tools/probe-about-placement.ps1* stands a plain window in for the host and drives it against each screen edge, off each edge, and onto a monitor whose origin is negative — this machine has one, at x 2560, y −497, which is exactly where the old code's `SPI_GETWORKAREA` fallback would have reported the primary monitor's rectangle and been wrong. Each case also records where the old arithmetic would have put the box, because a check that passes equally before and after a fix has not tested the fix: **six of the ten land outside the work area**, and four needed no clamping and got none.
...
Also covered: text set along a path, shearing one child of a group without disturbing its siblings, duplicating a sheared object and editing the duplicate, and copying one into another document. A graphic style carrying the effect to another object is **untested**, not covered: Illustrator has no scripting call that creates a graphic style, so the probe cannot set one up and records the case as untested rather than passing it. This paragraph listed it as covered through 0.1.1, which the evidence never said.
...
Absent against loaded-but-unused is four of six against four of six: Fisher's exact test gives p = 1.000 on the counts, and an exact rank test on cycles reached gives p = 0.494. The arm that exercised the effect crashed *less* often rather than more, and that difference is not significant either — p = 0.567 on counts, p = 0.275 on cycles. No pair of arms is separated by anything.

**What an experiment this size can and cannot settle**, because the numbers above are easy to over-read in either direction. Six trials per arm can detect only a large difference. The smallest two-tailed p reachable on crash counts at this size is 0.0022, and it requires one arm to crash in every trial while another crashes in none; since the plugin-absent arm crashed four of six, no possible result in either other arm could have reached 0.05 on counts alone. Cycles reached is the more powerful comparison and is the one this conclusion rests on — it reaches p = 0.002 on complete separation — and it found nothing. So a large effect in either direction is excluded. A small one is not, and a null result at this size is not a demonstration that the plugin is irrelevant.

During this sprint the full suite crashed three times, at three different fault offsets — `0xdfdd45`, `0x849dee`, and the earlier `0x18162a7` — always well into a long run. Twice it happened at the same probe, which looked like a reproducible sequence, so it was controlled: the same composition-then-save work, with the effect and without it, alternated, each trial in a fresh Illustrator. **No trial in either arm crashed** — which rules the sequence out as a reliable trigger, and nothing further. Six trials that produce no events at all cannot compare one arm against the other, and when the experiment was first run two of the six did not run to the end for reasons of their own (3 trials per arm, 2 of 3 finishing in each). It is repeated against each release's binary; for 0.1.2 and for 0.1.3 all six ran to the end and none crashed. What they establish is the narrow thing: the failure that had just happened twice running did not happen when the same sequence was driven deliberately. The reading that it is cumulative session load instead comes from the paragraph below, not from these six trials. [evidence/sequence-crash.txt](evidence/sequence-crash.txt).
...
4. **Run the crash comparison at a size that could find a small difference.** The question as it stood — whether three runs per arm was the honest end of it — is answered: it was not, and 0.1.1 replaced it with three arms of six trials of sixty cycles, which found no difference between the plugin absent and the plugin loaded. Six per arm still only excludes a large effect. Separating a modest one needs tens of trials per arm and days of machine time, and nothing in the product depends on the answer.
...
Every file under *evidence/* except two was written after the build recorded in [evidence/build.txt](evidence/build.txt), against the binary that hash names. The two exceptions are **native-shear.tsv** and **build.tsv**, and each earns it by not describing this binary at all. *native-shear.tsv* records what Illustrator's own *Shear* command does — it is the oracle this plugin is measured against, not a measurement of it, so a rebuild cannot invalidate it and only a new Illustrator could. *build.tsv* is the build probe's own result, written in the same instant as the record it would otherwise be compared against: seven milliseconds earlier, in this release, which a strict comparison reads as older than itself.

Nothing else is exempt, and **sequence-crash.tsv** used to be. It was on the list because that probe is slow and runs outside the suite — a reason to remember to run it, not a reason for the gate to stop asking — and the cost was what such a list always costs: a crash comparison measured against an older build rode through 0.1.0 as though it described the binary being shipped, and beside it sat an entry for **crash-ab.tsv**, a probe that had never been run at all. *sequence-crash.tsv* is measured against this build. The crash comparison is not, and it is not exempt either: for 0.1.2 its three files moved to *evidence/history/* with their date in the name, which is where a run kept for the record belongs, and every citation of them says they describe 0.1.1. A file earns an exemption by not describing the plugin binary, and by nothing else.

For 0.1.3 the whole suite ran once against the installed binary, and one probe had to be run again on its own. Illustrator died three times during the suite, each at `0x18162a7`: once in the persistence probe, which passed its second attempt, 5 of 5, and on both attempts of the export probe. The suite now starts Illustrator without activating it, so its restarts do not take the focus from a person at the machine; at first it started it minimized, and the export probe then died three more times on its own, five of five in all, each at a different step. Started shown, as in every earlier run, it passed the first time, 12 of 12. The effect matrix, which runs outside the suite, died at the same offset on the whole list both times it ran, with Illustrator shown, so the window state is not shown to matter. Its 23 cases were run through the probe's own *-Case* parameter, six to a fresh Illustrator, one of the four groups needing a second attempt, and the rows joined under one header in the probe's order; they are the rows 0.1.2 recorded from the whole list in one session, unchanged. The missing-plugin probe needs *tools/dismiss-warnings.ps1* running, because opening a document without the plugin raises a modal warning that blocks the script that opened it; started late, it answered that warning and the probe passed, 8 of 8. The sequence trials all ran to the end, and the About placement passed 15 of 15. Both matrices were regenerated afterward.

For 0.1.2 the whole suite ran once against the installed binary, and one probe had to be run again on its own. Illustrator died three times during that run, each time with an access violation at `0x18162a7`, the same host crash section Q describes: twice in the persistence probe, which saves, closes, and reopens documents, and once in the export probe, which opens exported files back in. The suite gives a probe a second attempt in a fresh Illustrator when the host stops answering. Export passed on its second attempt, 12 of 12. Persistence crashed on both, left its 0.1.1 files in place, and printed only an RPC failure, which is the stale-evidence case the paragraphs below describe. It was run again by itself afterward, in a fresh Illustrator, against the same installed binary: 5 passed, 0 failed. Both matrices were then regenerated, so they read the new persistence rows rather than the old ones.

For 0.1.1 that evidence again came from more than one pass of the suite, and the timestamp rule above is what makes that safe to say. Nothing failed in that run: fourteen probes ran, and then Illustrator was handed to another session working on a different plugin, because one machine has one copy of it. The rest ran afterward with *tools/run-release-suite.ps1* `-From schema`, which was new in 0.1.1 and starts at a named probe instead of the beginning. A skipped probe keeps the evidence it already had — those rows were measured against this same binary — and a `-From` that matches no probe stops the run rather than skipping everything and finishing with an empty summary that reads like success. Two probes were then run a third time, *blend* and *dialog*, because their evidence carried the workspace folder's name and the redaction that should have removed it had to be fixed first; both came back with the counts they had before, 12 and 17.

0.1.0's run was messier, and the same rule covered it. The machine was short of memory and the suite was killed partway through twice; the probes it had not reached were then run one at a time, which is what `-From` now exists to avoid. Three had to be re-measured for reasons that were not the plugin: the release matrix's `patternFill` case, the fills probe, and the everyday-use probe all died on the same `app.paste()` failure, a clipboard race on a machine several applications were sharing, and all three came back clean afterward — `patternFill` at a difference of 0.00e+00 from the native result. The blend probe and two dialog checks were re-run because the first pass was started without `LIVESHEAR_LOG` set, and without the trace there is no way to see whether Illustrator called the interpolation handler at all. Every one of these ran against the same installed binary, whose hash is compared with the build record before anything is packed.

That distinction is worth making because it has already caught something. In the run for 0.1.0 the persistence probe printed its heading, died on an RPC failure when Illustrator dropped the connection, and left its previous results in place; the suite's summary showed nothing wrong, and the matrix would have quoted rows measured against a different binary as though they described this one. Comparing each evidence file's timestamp against the build record is what found it. That check belongs in the release sequence, not in a person's memory.
...
Every path in the evidence is written through a redaction step, so nothing in it names the machine it was measured on. That was not true until this release, and the way it failed is worth keeping. The redaction replaces the paths this machine knows the spelling of, and ends with a net that redacts anything still shaped like an absolute path — which needs a drive letter to recognize one. A session working directory is encoded as a single folder name, every character that is not a letter or a digit becoming a dash, and in that spelling the workspace folder matched no rule; by the time the net could have caught it, the rule for the temporary directory had already replaced the drive letter with a placeholder. Sanitizing the front of a string had disarmed the check on the rest of it. Two files, *blend.txt* and *dialog.txt*, carried the name of the folder this repository sits in through the whole of 0.1.0. Both the redaction and the workspace privacy gate now know that spelling, and both probes were re-run rather than edited, because evidence that has been corrected by hand is no longer evidence.
