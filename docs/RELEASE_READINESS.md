...
**The recorded commit is the published one.** The build record names `a370ca3975bb4245536659448fdeb6cb73d1387c`, *Release 0.1.2*, built on a clean tree. This repository's history was rewritten on 2026-09-15 to remove absolute paths that earlier evidence had recorded, which is why the 0.1.1 record named a hash that no longer exists; this commit came after that rewrite, so the hash in the record is the hash on GitHub.
...
During this sprint the full suite crashed three times, at three different fault offsets — `0xdfdd45`, `0x849dee`, and the earlier `0x18162a7` — always well into a long run. Twice it happened at the same probe, which looked like a reproducible sequence, so it was controlled: the same composition-then-save work, with the effect and without it, alternated, each trial in a fresh Illustrator. **No trial in either arm crashed** — which rules the sequence out as a reliable trigger, and nothing further. Six trials that produce no events at all cannot compare one arm against the other, and when the experiment was first run two of the six did not run to the end for reasons of their own (3 trials per arm, 2 of 3 finishing in each). It is repeated against each release's binary; for 0.1.2 all six ran to the end and none crashed. What they establish is the narrow thing: the failure that had just happened twice running did not happen when the same sequence was driven deliberately. The reading that it is cumulative session load instead comes from the paragraph below, not from these six trials. [evidence/sequence-crash.txt](evidence/sequence-crash.txt).
