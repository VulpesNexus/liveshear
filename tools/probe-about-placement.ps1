...
.DESCRIPTION
    ...
    inside the work area of whatever monitor that lands on. Until 0.1.1 it did
    only the first half, so an Illustrator against a screen edge could put the
    box half off the desktop or under the taskbar -- the same defect the Shear
    dialog had until 0.1.0-rc.6, and the About box kept because the two windows
    each had their own copy of the placement code. They now share one, in
    plugin\Source\DialogPlacement.h, and this is what checks it.
    ...
    The harness is built here rather than committed. It is built into a
    directory whose path is plain ASCII: this repository can live under a path
    containing characters the C++ toolchain's own console cannot represent, and
    cmd.exe cannot even change directory into one.
...
if (-not $LogPath) { $LogPath = Join-Path $repo 'docs\evidence\about-placement.tsv' }

Start-ProbeResults -Probe 'about-placement'
...
$stage = Join-Path $env:TEMP 'liveshear-about-harness'
$report = Join-Path $env:TEMP 'liveshear-about-placement.tsv'
...
# A no-owner run first, which is also how the dialog's size is learned.
$cases.Insert(0, (New-Case 'no host window at all' 'ordinary' $null 0 0))

foreach ($case in $cases) {
    Write-Output "  $($case.Name)"
    $r = Invoke-Case -Case $case
    if ($null -eq $r) {
        Add-ProbeResult -Group $case.Group -Case $case.Name `
    ...
    if ($null -ne $case.Screen) {
        ...
        # Only the cases where the two differ say anything about the fix.
...
Save-ProbeResults -Path $LogPath
Write-Output ""
