...
SOURCES = [
    ...
    ("about-placement.tsv", "Where the About window opens", "The About box centered on Illustrator and stopped there, so a host against a screen edge could put it off the desktop or under the taskbar. It now shares its placement code with the Shear dialog. Measured without Illustrator: a plain window stands in for the host, because where the box opens is decided entirely by that window's rectangle and the monitor it falls on. Each case also records where the code before 0.1.1 would have put it, since a check that passes equally before and after a fix has not tested the fix."),
    ...
    ("bounds-flags.tsv", "Which flags ask for which box", "The SDK header is wrong twice about these, and the effect believed it for a sprint. This is the table the host actually returns, against fixtures chosen so each flag has something to do, with Illustrator's own geometricBounds beside it as the answer to match."),
