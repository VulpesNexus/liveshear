 ...
 * builds the very same ShearAbout.cpp and the very same dialog resource into a
 ...
 * actually landed. tools\probe-about-placement.ps1 drives the two together.
 *
...
const wchar_t* const kDialogCaption = L"About Shear";
...
HWND MakeOwner(HINSTANCE instance, int x, int y, int w, int h)
{
    ...
    wc.lpszClassName = L"ShearAboutHarnessOwner";
    RegisterClassExW(&wc);

    HWND owner = CreateWindowExW(0, L"ShearAboutHarnessOwner",
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, LPWSTR cmdLine, int)
{
    ...
    const bool shown = ShearShowAboutDialog(instance, owner, theme);

    if (owner != nullptr) DestroyWindow(owner);

    if (!shown) {
        if (reporting) return 1;
        MessageBoxW(nullptr, L"ShearShowAboutDialog returned false: the "
