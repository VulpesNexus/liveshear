 ...
 * two-hundred-line executable, so the layout can be looked at, and looked at
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, LPWSTR cmdLine, int)
{
    ...
    if (autoCloseMs > 0) {
        struct Closer {
            static DWORD WINAPI Run(LPVOID ms) {
                ...
                HWND dlg = FindWindowW(nullptr, L"About Shear");
    ...
    if (!ShearShowAboutDialog(instance, nullptr, theme)) {
